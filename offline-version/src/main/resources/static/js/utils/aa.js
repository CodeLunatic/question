...
/**
 xuejianxin@20170505
 ajaxAnywhere.postJson(
 {
       "url"       :  baseUrl+"postJson.do" //必须
     ,"data"       :  JSON2.stringify({"id":"111","name":"哈哈哈"}) //必须
     ,"zones"      : "postJsonZone" //必须
     ,"contentType": "application/json; charset=UTF-8" //必须
 }
 );
 //后台写法  @RequestBody 注解是必须的，表示把请求的正文转化为对象
 @RequestMapping("/postJson")
 public String postJson(@RequestBody Demo demo,Model m) {
		m.addAttribute("demo", demo);
		return "/demo/list";
  }
 */
AjaxAnywhere.prototype.postJson = function (options) {
    if (!options.url) {
        alert("url 不能为空!");
        return false;
    }
    ...
    if (options.zones != null && options.zones != "") {
        url += '&aazones=' + encodeURIComponent(options.zones);
    }
    this.req.open("POST", url, this.isAsynch());
    this.req.setRequestHeader("Content-Type", options.contentType);
